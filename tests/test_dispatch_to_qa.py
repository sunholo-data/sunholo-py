...
@patch('sunholo.agents.dispatch_to_qa.ConfigManager')
def test_prep_request_payload(mock_config_manager, mock_route_endpoint, user_input, chat_history, vector_name, stream, expected_endpoint, expected_payload):
    ...
    mock_config.vacConfig.side_effect = lambda key: "langserve" if key == "agent" else "langserve" if key == "agent_type" else None
