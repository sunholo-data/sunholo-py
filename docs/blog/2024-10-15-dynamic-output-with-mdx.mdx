...
Another situation we need to deal with non-deterministic GenAI output is that it will likely fail, and we need to make sure that if it outputs invalid Components it doesn't bork the entire text.  This wrapper ensures that even if the GenAI model outputs invalid code, we still see something:

```js
const SafeComponent = ({ children, fallback = <span>Invalid Component</span> }) => {
  try {
    return children;
  } catch (error) {
...
> I'm scaling the Cloud Run to 0 for this example so if you try it be patient: on a cold start the first response will be a little slower than subsequent ones.  Its also not saving any chat history.
