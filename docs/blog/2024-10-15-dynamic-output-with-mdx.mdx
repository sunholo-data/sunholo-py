...
If you hadn't guessed already, the above code was created by a GenAI model.  It then seems viable to request a model to output React components, and if that text is within an environment that supports its display, we will instead render the component instead of the text.
I also like that we control what is rendered, by specifying the components by passing them in at runtime, and we can configure those components to not need to many arguments, to make it as easy as possible for the model to render: we only need to ask nicely in its prompt.
...
It just so happens, that I had a prototype GenAI Chat React Component lying around as one of [Multivac's UI options](/docs/multivac/#user-interfaces), and can use it to stream custom GenAI APIs, so I'll attempt to host that Chat UI, ask it to output MDX format, and then render them within the blog.
...
- Even if you do create them dynamically, Components that run during development may not work during build, as they depend on website elements (e.g. Plot.ly)
